#' Estimate Direct Model
#'
#' Given \code{y} and \code{xreg}, this function estimates direct models as
#' described bellow in the details section.
#'
...
#' the first equation. Allowing for \code{p_start} and \code{q_start} to vary
#' implies more models estimated because it opens up
#' more possibilities.
#'
#'
...
#' \item{h}{The horizon over which y is to be forecast.}
#'
#' @export
#'
...
#' Direct Autoregression Model
#'
#' Estimates a direct autoregression model.
#'
#' @param y A vector or a time series.
#' @param h An integer, the horizon over which \code{y} is to be forecast.
...
#' more information. Forecasting obtainded by the direct autoregression model
...
#' the functions \code{\link{model_est}} and \code{\link{model_est_single}}.
#'
#' @param models An object generated by the funcions
#' \code{\link{model_est}} and \code{\link{model_est_single}}.
...
#'
#' @return A projection value for the horizon \code{h} considered in the
...
#'
#' This function calculates root mean square error or mean absolute error for
#' a model estimated by the function \code{\link{model_fcast}}. This function
#' is appropriate for forecasting evaluation of
#' core inflation measures that are never revised.
#'
#' @param k An integer to determine the number of observations to include
#' in the out-of-sample set.
#' @param y A numeric vector or time series for dependet variable, tipplically
#' headline inflation.
#' @param xreg A numeric vector or time series for independent variable, a measure
#' of core inflation that is never revised.
#' @param lags A data frame generated by the function \code{\link{lags}}.
#' @param h An integer to set the desired horizon.
#' @param RMSE Logical: \code{TRUE} for \code{RMSE} and \code{FALSE} to \code{MAE}.
#'
...
#'
#' Calculates root mean square error or mean absolute error for
#' a univariate model estimated by the function \code{\link{model_fcast}} and
...
#'
#' \code{model_error_wave} estimates the prediction error for a model
#' fitted by the function \code{\link{model_est}} where the \code{x} variable
...
#' @param RMSE logical: \code{TRUE} for \code{RMSE} and \code{FALSE} to \code{MAE}.
#'
#' @return A vector with error of every wavelet model considered.
#'
...
#'
#' This function calculates the mean of forecast errors for several horizons.
#' Those errors are estimated by the function \code{\link{model_error_wave}}.
#'
#' @param h An positive integer to set the maximum horizon (\eqn{1, 2, ..., h}).
...
#' @param ... Additional arguments to pass for functions:
#' \itemize{
...
#' error for the model fitted by the
#' function \code{\link{model_est}} where the \code{x} variable is
